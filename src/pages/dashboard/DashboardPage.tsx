    ...
    <div className="space-y-8 animate-fadeIn text-slate-700 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, idx) => {
          ...
          const isAppCard = stat.title === "Postulaciones Activas";
          return (
            <div 
              key={idx} 
              onClick={() => {
              ...
              className={`bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between ${
                isAppCard ? "cursor-pointer hover:border-[#5D8CE2]/40 hover:shadow-md transition-all duration-200" : ""
              }`}
            >
          ...
          <div className="flex flex-col gap-4 border-b border-gray-100 pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h2 className="font-display font-bold text-xl flex items-center gap-2 text-[#00135B]">
                ...
                <span>{activeTab === "applications" ? "Mis Postulaciones" : "Programas Recomendados"}</span>
              </h2>

              {/* General Filters: Todos / Favoritos / Mis Postulaciones */}
              <div className="flex gap-1 bg-slate-100 p-1 rounded-xl border border-gray-200/50 self-start">
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${activeTab === "applications" ? "bg-white text-[#00135B] shadow-sm" : "text-slate-500 hover:text-slate-800 bg-transparent border-none"}`}
                >
            ...
            {/* Program Type Pills — Hide when viewing applications */}
          ...
          {activeTab === "applications" ? (
            applications.length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-gray-200 shadow-sm text-center space-y-3">
                ...
                <h3 className="font-bold text-slate-600 text-sm">No tienes postulaciones activas</h3>
                <p className="text-xs text-slate-400 max-w-xs mx-auto">
                  Explora los programas recomendados y haz clic en "Iniciar mi postulación" para comenzar tu camino.
                </p>
              </div>
            ) : (
              <div className="space-y-4 animate-fadeIn">
                {applications.map((app) => (
                  <div 
                    key={app.id} 
                    className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 text-left hover:shadow-md transition-shadow relative overflow-hidden"
                  >
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`text-[8px] px-2.5 py-0.5 rounded-full font-extrabold uppercase tracking-wider ${
                          app.program_type === 'scholarship' ? 'bg-emerald-50 text-emerald-700 border border-emerald-250' : 
                          app.program_type === 'volunteering' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 
                          'bg-violet-50 text-violet-700 border border-violet-200'
                        }`}>
                          {app.program_type === 'scholarship' ? 'Beca' : app.program_type === 'volunteering' ? 'Voluntariado' : app.program_type === 'exchange' ? 'Intercambio' : app.program_type}
                        </span>
                        ...
                        <span className={`text-[8px] px-2.5 py-0.5 rounded-full font-extrabold uppercase tracking-wider ${
                          ...
                          app.status === 'in_review' ? 'bg-blue-100 text-blue-800 border border-blue-300' :
                          app.status === 'pending' ? 'bg-amber-100 text-amber-800 border border-amber-300' :
                          'bg-slate-100 text-slate-600 border border-slate-300'
                        }`}>
                          {app.status === 'accepted' && 'Aceptado 🎉'}
                          {app.status === 'rejected' && 'Rechazado'}
                          {app.status === 'in_review' && 'En Revisión'}
                          {app.status === 'pending' && 'Enviado - Pendiente'}
                          {app.status === 'started' && 'Borrador'}
                        </span>
                      </div>

                      <h3 className="font-extrabold text-lg text-[#00135B] font-display leading-tight">
                        {app.program_title}
                      </h3>
                      
                      <p className="text-[11px] text-slate-500 font-semibold">
                        Organiza: <span className="font-bold text-slate-700">{app.program_organization}</span>
                        <span className="mx-2 opacity-50">|</span>
                        {app.applied_at || app.created_at ? (
                          <span>Fecha: <span className="font-bold text-slate-700">{new Date(app.applied_at || app.created_at).toLocaleDateString()}</span></span>
                        ) : null}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => navigate((app.program_type === 'scholarship' ? '/becas/' : '/opportunities/') + app.program_slug)}
                        className="px-4 py-2 rounded-xl border border-gray-250 bg-white hover:bg-slate-50 text-[#00135B] text-xs font-bold transition-all cursor-pointer shadow-sm active:scale-95"
                      >
                        {app.status === 'started' ? 'Continuar Postulación' : 'Ver Detalles'}
                      </button>
                    </div>
                  </div>
                ))}
                    ...
                    >
                      ...
                      <div className="relative z-10 w-full p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="space-y-3.5 flex-1 text-left">
                          ...
                          {/* Title */}
                          <h3 
                            onClick={() => navigate((prog.type === 'scholarship' ? "/becas/" : "/opportunities/") + (prog.slug || "aiesec-voluntariado"))}
